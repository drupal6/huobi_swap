...
class MatPlot:
    ...
    @classmethod
    def show(cls, df):
        df_simple = df[['5min_hist', '5min_z', '5min_close']]
        X_train, X_test, Y_train, Y_test = train_test_split(df_simple.iloc[:, :2], df_simple["5min_close"], train_size=.80)
        print("原始数据特征:", df_simple.iloc[:, :3].shape,
              ",训练数据特征:", X_train.shape,
              ",测试数据特征:", X_test.shape)

        print("原始数据标签:", df_simple["5min_close"].shape,
