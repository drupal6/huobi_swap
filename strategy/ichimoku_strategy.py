...
class IchimokuStrategy(BaseStrategy):
    ...
    def calculate_signal(self):
        ...
        df["conversion_min"], df["conversion_max"] = talib.MINMAX(df["close"], self.conversion_periods)
        df["conversion"] = (df["conversion_min"] + df["conversion_max"])/2
        df["base_min"], df["base_max"] = talib.MINMAX(df["close"], self.base_periods)
        df["base"] = (df["base_min"] + df["base_max"]) / 2
        df["leada"] = (df["conversion"] + df["base"]) / 2
        df["leadb_min"], df["leadb_max"] = talib.MINMAX(df["close"], self.lagging_span2_periods)
