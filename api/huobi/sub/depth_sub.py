...
class DepthSub(BaseSub):
    ...
    def __init__(self, strategy, depth_size=10):
        """
        strategy:策略类
        ...
        self._step = self._strategy.step
        self._depth_size = depth_size
        self._depths_max_size = self._strategy.depths_max_size
        self._ch = "market.{s}.depth.{d}".format(s=self._symbol.upper(), d=self._step)
        self._strategy.depths[self._ch] = deque(maxlen=self._depths_max_size)
    ...
    async def call_back(self, channel, data):
        ...
        depths = self._strategy.depths[channel]
